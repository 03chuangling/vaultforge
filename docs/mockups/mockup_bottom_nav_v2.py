#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""VaultForge 首页三 Tab 改版 · 效果图（草案 v2：拉取入口移至设置）"""
import os, math
...
yy = sy + 164
els = [("GitHub", "octocat · github.com"),
       ("阿里云控制台", "root · console.aliyun.com"),
       ("微信", "wxid_8f***2k"),
       ("宝塔面板", "bt · 154.89.149.92:8888"),
       ("网易邮箱", "user@163.com")]
for name, sub in els:
    ...
    for bt in ["复制", "显示"]:
        bx -= 42
        RR(bx, yy + 19, 42, 26, 13, fill="#233029")
        T(bx + 21, yy + 26, bt, 11, "#9FD8C6", anchor="ma")
        bx -= 8
    yy += 74
...
# ===== 标题 / 图注 =====
T(765, 34, "VaultForge · 首页底部导航三 Tab 改版 · 效果图（草案 v2）", 26, TEXT, bold=True, anchor="ma")
T(765, 76, "① 秘钥仓（原有页面） ② Bitwarden 密码库（条目浏览） ③ 设置（新增拉取入口） ｜ 深色墨绿冷灰主题", 13, TEXT2, anchor="ma")
for i, lab in enumerate(["① 秘钥仓（原有页面）", "② Bitwarden 密码库（浏览）", "③ 设置（拉取入口）"]):
    cx = 60 + 490 * i + 215
    T(cx, 1066, lab, 17, "#C7D6D1", bold=True, anchor="ma")
T(765, 1102, "v2 变更：密码库「拉取」入口移至设置 ｜ 密码库 Tab 专注条目浏览（显示 / 复制）；未拉取时显示引导空状态", 13, TEXT3, anchor="ma")
T(765, 1132, "待确认以上布局；细节（图标 / 文案 / 区块顺序）可再调 —— 确认后开始开发。", 12, TEXT3, anchor="ma")

out1 = "/sdcard/Download/Operit/vaultforge/mockup_bottom_nav_v2.png"
out2 = "/sdcard/Download/mockup_bottom_nav_v2.png"
