#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""VaultForge 首页三 Tab 改版 · 效果图（草案 v1）渲染脚本"""
import os, math
...
T(sx + 24, sy + 82, "自建 Vaultwarden · 主密码不落盘", 12, TEXT2)
fields = [("服务器地址", "https://bit.bdshjgg.com"),
          ("邮箱", "i@bdshjgg.com"),
          ("主密码", "••••••••••••")]
yy = sy + 108
for label, val in fields:
    T(sx + 24, yy, label, 12, TEXT3)
    RR(sx + 24, yy + 16, sw - 48, 48, 14, fill=CARD2, outline=BORDER, width=1)
    T(sx + 36, yy + 31, val, 14, TEXT)
    yy += 78
RR(sx + 24, yy, sw - 48, 52, 16, fill=ACC2)
T(sx + sw / 2, yy + 26, "拉取密码库", 16, WHITE, bold=True, anchor="mm")
yy += 68
T(sx + 24, yy, "已拉取 42 条 · 3 个文件夹，可一键导入", 11, TEXT3)
yy += 18
for name, sub in [("GitHub", "octocat · github.com"), ("阿里云控制台", "root · console.aliyun.com")]:
    RR(sx + 24, yy, sw - 48, 58, 14, fill=CARD, outline=BORDER, width=1)
    T(sx + 42, yy + 9, name, 14, TEXT, bold=True)
    T(sx + 42, yy + 32, sub, 11, TEXT2)
    bx = sx + sw - 42
    for bt in ["复制", "显示"]:
        bx -= 42
        RR(bx, yy + 16, 42, 26, 13, fill="#233029")
        T(bx + 21, yy + 23, bt, 11, "#9FD8C6", anchor="ma")
        bx -= 8
    yy += 70
RR(sx + 24, yy, sw - 48, 46, 14, outline=ACC, width=1.5)
T(sx + sw / 2, yy + 23, "全部导入", 14, ACC, bold=True, anchor="mm")
T(sx + sw / 2, sy + sh - 112, "主密码仅本次使用，不会保存", 11, TEXT3, anchor="ma")
...
yy = section(sy + 108, "通用", [("主题", "t", "跟随系统 ›"), ("进入自动检测", "switch", "")])
...
# ===== 标题 / 图注 =====
T(765, 34, "VaultForge · 首页底部导航三 Tab 改版 · 效果图（草案 v1）", 26, TEXT, bold=True, anchor="ma")
T(765, 76, "① 秘钥仓（原有页面） ② Bitwarden（提为独立 Tab） ③ 设置 ｜ 深色墨绿冷灰主题", 13, TEXT2, anchor="ma")
for i, lab in enumerate(["① 秘钥仓（原有页面）", "② Bitwarden（独立页）", "③ 设置"]):
    cx = 60 + 490 * i + 215
    T(cx, 1066, lab, 17, "#C7D6D1", bold=True, anchor="ma")
T(765, 1108, "待确认：底部三 Tab 结构 ｜ Bitwarden 页内容与交互 ｜ 设置页保留项 —— 确认后开始开发", 14, TEXT3, anchor="ma")
T(765, 1138, "注：本图为设计草案，最终以你的确认稿为准。", 12, TEXT3, anchor="ma")

out1 = "/sdcard/Download/Operit/vaultforge/mockup_bottom_nav_v1.png"
out2 = "/sdcard/Download/mockup_bottom_nav_v1.png"
